...
// Ported from Product-Studio-v9.37.01/proxy/routes/v1/traces.js — same
...
module.exports = function tracesRouterFactory(supabaseAdmin) {
   ...
   *   post:
   *     tags: [traces]
   *     summary: Create a trace explicitly
   *     description: Idempotent on (company_id, app_id, client_trace_id), strict-compare on agent_name/session_id.
   *     security: [{ bearerAuth: [] }]
   *     requestBody:
   *       required: true
   *       content:
   ...
   *           schema:
   *             type: object
   *             required: [agent_name, client_trace_id]
   *             properties:
   *               agent_name: { type: string }
   *               client_trace_id: { type: string, format: uuid }
   *               session_id: { type: string, nullable: true }
   *               product_id: { type: string, nullable: true }
   *               outcome_id: { type: string, format: uuid, nullable: true }
   *     responses:
   *       200: { description: Created (or deduplicated replay). }
   *       400: { description: Missing field, or outcome_id not owned. }
   *       409: { description: Replayed client_trace_id with a different agent_name/session_id. }
   */
  router.post('/traces', requireIngestionScope('traces'), async function (req, res) {
    const body = req.body || {};
    ...
    if (result.error) {
      ...
      // 23503 = foreign_key_violation — e.g. a bad outcome_id.
   ...
   *   patch:
   *     tags: [traces]
   *     summary: Mark a trace completed
   *     security: [{ bearerAuth: [] }]
   *     parameters:
   *       - { name: id, in: path, required: true, schema: { type: string, format: uuid } }
   *     responses:
   *       200: { description: Completed. }
   *       404: { description: Trace not found or not owned. }
   *   get:
   *     tags: [traces]
   ...
   *       404: { description: Trace not found or not owned. }
   */
  // PATCH /v1/traces/{id} — sets completed_at only. Ownership-scoped by
  // (company_id, app_id), same discipline as PATCH /v1/outcomes/{id}.
  router.patch('/traces/:id', requireIngestionScope('traces'), async function (req, res) {
   ...
   *   get:
   *     tags: [traces]
   *     summary: Read every span in a trace, in sequence order
   *     description: Embeds the mt_ai_usage_events row for llm_call spans (null for tool_call spans).
   *     security: [{ bearerAuth: [] }]
   *     parameters:
   *       - { name: id, in: path, required: true, schema: { type: string, format: uuid } }
   *     responses:
   *       200: { description: Ordered list of spans. }
   *       404: { description: Trace not found or not owned. }
   */
  ...
  // usage_event_id) — null for a tool_call span, which has no usage event.
  // Ownership checked against the trace itself FIRST, so a trace this
  // credential doesn't own returns a plain 404 rather than leaking whether
  // it exists via its spans.
