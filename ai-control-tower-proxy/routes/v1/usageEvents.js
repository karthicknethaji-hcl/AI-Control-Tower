...
// Ported from Product-Studio-v9.37.01/proxy/routes/v1/usageEvents.js — same
...
// This allowlist keeps an external caller from putting arbitrary content —
// including full prompt/response text — inside this JSONB blob; that would
// bypass the invariant that model-visible content only ever enters through
// the payload-gated /v1/trace-payloads route.
//
// Each provider's table is traced to that provider's own adapter, not to
// provider documentation. cache_creation_input_tokens/service_tier
...
// controlled only by the total-size backstop below.
...
const PROVIDER_USAGE_RAW_ALLOWLISTS = {
  ...
  gemini: {
    ...
    // element shape not confirmed against live Gemini docs/traffic — accept
    // any array.
    input_tokens_by_modality: { type: 'array' }
  }
};

// A plain `obj[key]` lookup on a caller-controlled string resolves
// inherited Object.prototype members (e.g. key === 'constructor') instead
// of undefined for any key this codebase never defined on that object —
// which would silently defeat every `if (!rule)` unrecognized-key
// rejection below. `_ownGet` restores the intended "only a key this table
...
// provider falls back to Anthropic's table.
function _validateProviderUsageRaw(providerUsageRaw, provider) {
  ...
  // Normalized (lowercased/trimmed) for table selection only — the raw
...
function _validateItem(item) {
  ...
  // whole query, incorrectly marking every OTHER item's legitimate
  // outcome_id as unowned too.
  if (item.outcome_id != null && !UUID_RE.test(String(item.outcome_id))) {
    return 'outcome_id must be a valid UUID.';
  }
  // client_trace_id is the only trace-continuation key; agent_name is
  // required whenever it's present, same rule the RPC itself enforces —
  // checked here too so a caller gets a clean 400 instead of a
  // database-level error surfacing as a 500.
...
// last_activity_at auto-bump — ownership already verified by the caller
// before this runs (see _fetchOwnedOutcomeIds), so this is a plain scoped
// update, not a re-check. Fire-and-forget, never awaited by the request
// path. A mismatched outcome_id was already rejected earlier in the
// request, so silent failure here only ever means a genuine best-effort
// miss, never a masked ownership gap.
...
// independent DB round trips must not run fully sequentially (up to 500x
// latency for no correctness reason — these are independent records, not a
// transaction) nor fully unbounded (500 simultaneous connections would
// overwhelm Supabase's pooler). limit caps how many items are ever
// in-flight at once; order of the returned array always matches input
// order regardless of which item resolves first.
...
// batch, instead of one ownership-check query per item.
...
// ownedOutcomeIds is pre-fetched once per batch rather than re-checked per
// item — without this check at all, a caller-supplied outcome_id from a
// DIFFERENT (company_id, app_id) would still satisfy the plain FK on
// mt_ai_usage_events.outcome_id (existence only, not ownership) and persist
// a permanent cross-tenant reference. Mirrors the same ownership guarantee
// PATCH /v1/outcomes/{id} enforces.
//
// This calls mt_ai_record_usage_event_with_span() so this route and any
// internal call site share exactly one place a usage event (and its
// optional span/trace) is ever written.
async function _processItem(supabaseAdmin, item, companyId, appId, ownedOutcomeIds) {
  ...
  const { data, error } = await supabaseAdmin.rpc('mt_ai_record_usage_event_with_span', buildUsageEventRpcParams({
    ...
    // route is the consumer-tier ingestion surface.
  ...
  if (error) {
    ...
    // genuine integration bug on the caller's side, mapped to 409.
    if (error.code === '23514') {
      return { error: { type: 'conflict', message: error.message } };
    }
    // 23503 = foreign_key_violation — e.g. a bad outcome_type_id — mapped
    // to 400 instead of an undifferentiated 500.
    if (error.code === '23503') {
...
module.exports = function usageEventsRouterFactory(supabaseAdmin) {
   ...
   *   post:
   *     tags: [usage-events]
   *     summary: Record a usage event, or a batch of up to 500
   *     description: >
   *       Single object OR array. Idempotent on (company_id, app_id, client_call_id).
   *       Each item is validated and processed independently; a single malformed
   *       item never discards the batch.
   *     security: [{ bearerAuth: [] }]
   *     requestBody:
   *       required: true
   *       content:
   ...
   *             oneOf:
   *               - type: object
   *                 required: [client_call_id, user_role_at_call, caller, requested_model, status, request_started_at]
   *                 properties:
   *                   client_call_id: { type: string, format: uuid }
   *                   user_role_at_call: { type: string }
   *                   caller: { type: string }
   ...
   *                 items: { type: object }
   *     responses:
   *       200: { description: 'Single result, or a results array for a batch.' }
   *       400: { description: Missing/invalid field, or batch size violation. }
   *       409: { description: Replayed client_call_id/client_trace_id with different identity fields. }
   */
  router.post('/usage-events', requireIngestionScope('usage'), async function (req, res) {
    const body = req.body;
    ...
    // not one per item. Only syntactically-valid UUIDs are collected here
    // (matches _validateItem's own check) — a malformed value is rejected
    // per-item before it ever reaches this shared query.
    ...
    // are independent cost/telemetry records, not a transaction, so there
    // is no correctness reason to process them one at a time. `index` (not
    // resolution order) is what maps a result back to its request
    // position, so out-of-order completion is never observable.
    ...
    // item — fires on both a fresh insert and a deduplicated replay,
    ...
    if (!isBatch) {
      ...
      if (only.error) {
        // 'conflict' is a new error type _processItem() can return (a
        // replayed client_call_id/client_trace_id with different identity
        // fields); mapped to 409, rather than the flat 400 every other
        // error type gets.
  ...
  /**
   * @openapi
   * /v1/usage-events/{client_call_id}/units-generated:
   *   patch:
   *     tags: [usage-events]
   *     summary: Report back the units_generated count for a usage event
   *     description: Idempotent, scoped to (company_id, app_id, client_call_id).
   *     security: [{ bearerAuth: [] }]
   *     parameters:
   *       - { name: client_call_id, in: path, required: true, schema: { type: string, format: uuid } }
   *     requestBody:
   *       required: true
   *       content:
   *         application/json:
   *           schema: { type: object, required: [units_generated], properties: { units_generated: { type: integer, minimum: 0 } } }
   *     responses:
   *       200: { description: Updated. }
   *       400: { description: units_generated must be an integer >= 0. }
   *       404: { description: No usage event found for this client_call_id. }
   */
  router.patch('/usage-events/:client_call_id/units-generated', requireIngestionScope('usage'), async function (req, res) {
   ...
   *   get:
   *     tags: [usage-events]
   *     summary: Read back usage events (keyset pagination)
   *     description: Reconciliation read-back, not a dashboard replacement.
   *     security: [{ bearerAuth: [] }]
   *     parameters:
   ...
   *       - { name: cursor, in: query, schema: { type: string }, description: Opaque base64 keyset cursor from a prior page's next_cursor. }
   *     responses:
   *       200: { description: '{ events: [...], next_cursor }' }
   *       400: { description: Missing/invalid start/end, or malformed cursor. }
   */
  router.get('/usage-events', async function (req, res) {
    ...
    if (req.query.cursor) {
      try {
        ...
        // get spliced into a raw PostgREST filter string below.
    ...
    // calculated_cost is application-computed here, not via an RPC — same
    // pricing-table shape and range-match logic as the read RPCs used
    // elsewhere in this repo, deliberately not reused directly since those
    // assume a human Supabase Auth session.
